...
/**
 *_getline - function to gets a line of string from a file
...
char *_strtok(char *str, const char *delimeter, int whichf)
{
	static char *save;
	char *_new = NULL;
	int k = 0, (*func)(char *, const char *), loc;
	...
	if (!str || !*str)
	{
		...
		while (func(save + k, delimeter) != 1 && *(save + k) != '\0')
			k++;
		if (*(save + k) == '\0')
		{
			_new = save, save = NULL;
	...
	while (func(str + k, delimeter) != 1 && *(str + k) != '\0')
		k++;
